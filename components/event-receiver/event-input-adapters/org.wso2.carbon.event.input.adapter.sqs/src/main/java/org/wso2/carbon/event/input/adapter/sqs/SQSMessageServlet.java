...
public class SQSMessageServlet extends HttpServlet {
    ...
    private static Log log = LogFactory.getLog(SQSMessageServlet.class);
    ...
    public SQSMessageServlet(InputEventAdapterListener eventAdaptorListener, int tenantId, String exposedTransports, boolean isBasicAuthEnabled) {
    ...
    private int checkAuthentication(HttpServletRequest req) {
        ...
        RealmService realmService = SQSEventAdapterServiceValueHolder.getRealmService();
        int tenantId;
        try {
    ...
    protected void doPost(HttpServletRequest req,
        ...
        if(exposedTransports.equalsIgnoreCase(SQSEventAdapterConstants.HTTPS)){
            if(! req.isSecure()){
                res.setStatus(403);
        ...
        }else if(exposedTransports.equalsIgnoreCase(SQSEventAdapterConstants.HTTP)){
            if(req.isSecure()){
                res.setStatus(403);
        ...
        SQSEventAdapter.executorService.submit(new HTTPRequestProcessor(eventAdaptorListener, data, tenantId));
